...
```{r}
  ...
  list.of.packages <- c("R.matlab","lubridate","tidyr","plotly","rasterly","stringr","xts","abind","bioRad","terra")
  ...
  SlopeFile         <- file.path(DataDirectory,"GIS","Slope.tif")
...
plot(mtrend,type="l")
```
Now calculate the temporal standard deviation of each pixel in the ftrend product
then get the probability distribution of the ftrend standard deviation
...
HighSDPixelSamples <- highSDPixels

#Set all the non-sampled cells to NA
HighSDPixelSamples[!cells(HighSDPixelSamples) %in% HighSDPixelsSampleCells$cell] <- NA
...
#Initialise the sample raster
LowSDPixelSamples <- LowSDPixels

#Set all the non-sampled cells to NA
LowSDPixelSamples[!cells(LowSDPixelSamples) %in% LowSDPixelsSampleCells$cell] <- NA
...
```{r}
SlopeData <- terra::rast(SlopeFile)
